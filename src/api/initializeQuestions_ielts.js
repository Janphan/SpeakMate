...
const ieltsBanks = [
    ...
    },
    {
        ...
        part2: [
            {
                cueCard: 'Describe a subject you enjoyed studying at school.',
                prompts: ['What the subject was', 'What you learned', 'Why you enjoyed it', 'How it has been useful to you'],
            },
        ],
        part3: [
    ...
    },
    {
        ...
        level: 'Band 6-7',
        part1: [
            'How did you choose your field of study or career path?',
        ...
        part2: [
            {
                cueCard: 'Describe a project or task you worked on that you are proud of.',
                prompts: ['What it was', 'How you approached it', 'What the outcome was', 'Why you are proud of it'],
            },
        ],
        part3: [
            'How is technology changing the nature of work in your field?',
            'What role should universities play in preparing students for the job market?',
            'How do you think remote work will affect organizations and work culture?',
    ...
    },
    {
        ...
        part1: [
            ...
            'What ethical frameworks should guide decision-making in your profession?',
        ],
        part2: [
            {
                cueCard: 'Describe a significant challenge you faced in your career or studies.',
                prompts: ['What the challenge was', 'How you addressed it', 'What you learned from it', 'How it changed your perspective'],
            },
        ],
        part3: [
            'How should educational institutions adapt to prepare students for an increasingly volatile job market?',
            'Evaluate the intersection of artificial intelligence and future employment patterns.',
            'What are the psychological and sociological impacts of the gig economy on workers?',
            'How do you envision the relationship between sustainability and profitability in future business models?',
...
export const initializeIeltsBanks = async () => {
    try {
        const questionsRef = collection(db, 'questions');
        for (const bank of ieltsBanks) {
            const docId = `${bank.topic}_${bank.level}`.replace(/\s+/g, '_');
            await setDoc(doc(questionsRef, docId), {
                ...bank,
                createdAt: new Date(),
            });
            logger.info(`Added IELTS bank: ${bank.topic} - ${bank.level}`);
        }
        logger.info(`✅ IELTS question banks initialized! (${ieltsBanks.length} banks)`);
        const topics = [...new Set(ieltsBanks.map(b => b.topic))];
        const levels = [...new Set(ieltsBanks.map(b => b.level))];
        logger.info(`📚 Topics: ${topics.length}, Levels: ${levels.join(', ')}`);
    } catch (error) {
        logger.error('Error initializing IELTS banks', { error: error.message });
    }
};
